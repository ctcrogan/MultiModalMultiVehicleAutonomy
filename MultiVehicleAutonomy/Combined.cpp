...
//Apply's vision filter to each frame !!Probable source of mem leak!!
...
IplImage* GetThresholdedImage(IplImage* imgHSV)
{
	...
	cvInRangeS(imgHSV, cvScalar(170,150,30), cvScalar(180,255,255), imgThresh); // red
	//cvInRangeS(imgHSV, cvScalar(104, 178, 70), cvScalar(130, 240, 255), imgThresh);// blue color
	// cvInRangeS(imgHSV, cvScalar(25, 150, 50), cvScalar(50, 255, 255), imgThresh);// yellow color
	return imgThresh;
}
...
void trackObject(IplImage* imgThresh)
{
	...
	//This is never even used, I don't know what its purpose was
	//supposed to be. 
	static int countering;

	
	//If the "area" exceeds the threshold then we consider it the target.
	//
	//I use scare-quotes around area because what this 
	//is really doing is computing the central moments of the filtered
	//black-and-white image, often referred to as the "area". This is
	//misleading in this context, however, because it is treated as if it
	//were the area of an object. This is not an ideal algorithm
	//for an object tracker because it is extremely susceptible 
	//to false positives generated by noise, particularly since it is in
	//black and white. This algorithm needs to be changed. 
	if(area > AREA_THRESHOLD)
	{
		// calculate the position of the ball
		//
		//Computes the centroid of the image. Again this is mistreated
		//as the center of the "object" when it is really just the 
		//mean of the coordinates of the white pixels in the
		//image. 
...
int runAerialVision()
{
	...
	{   
		...
		if(!found)
			trackObject(imgThresh);	//track the position of the object
		else
			break;
